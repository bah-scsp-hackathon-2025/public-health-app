...
async def test_enhanced_standard_agent():
    ...
    try:
        ...
        print(
            f"📈 Rising trends: {len(result.get('rising_trends', []))} trends detected"
        )
        print(
            f"🔬 Epidemiological signals: {len(result.get('epidemiological_signals', []))} signals"
        )
        print(
            f"🎯 Recommendations: {len(result.get('recommendations', []))} recommendations"
        )
        print(
            f"⚠️ Risk level: {result.get('risk_assessment', {}).get('overall_risk_level', 'unknown')}"
        )
        ...
        if result.get("alerts"):
            ...
            print(
                f"   Severity: {alert['severity']} | Priority Score: {alert['analysis']['priority_score']}/100"
            )
        ...
        if result.get("rising_trends"):
            ...
            print(
                f"   Direction: {trend['trend_direction']} | Change: {trend['change_percent']}%"
            )
        ...
        if result.get("risk_assessment"):
            ...
            print(
                f"   Population Affected: {risk.get('total_population_affected', 0):,}"
            )
...
async def test_enhanced_react_agent():
    ...
    try:
        ...
        if result.get("risk_assessment"):
            ...
            print(f"   Overall Risk: {risk.get('overall_risk_level', 'unknown')}")
            print(
                f"   Signals Analyzed: {risk.get('epidemiological_signals_analyzed', 0)}"
            )
...
async def compare_agents():
    ...
    if standard_result and react_result:
        ...
        print(
            f"  - Risk level: {standard_result.get('risk_assessment', {}).get('overall_risk_level', 'unknown')}"
        )

        print("\nReAct Agent (using epidemiological APIs):")
        print(
            f"  - Epidemiological signals: {len(react_result.get('epidemiological_signals', []))}"
        )
        print(
            f"  - Rising trends detected: {len(react_result.get('rising_trends', []))}"
        )
        print(f"  - Recommendations: {len(react_result.get('recommendations', []))}")
        print(
            f"  - Risk level: {react_result.get('risk_assessment', {}).get('overall_risk_level', 'unknown')}"
        )

...
class DashboardResponse(BaseModel):
    ...
    alerts: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Actual alert data with analysis and recommendations"
    )
    rising_trends: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Detected rising trends from time series analysis"
    )
    epidemiological_signals: Optional[List[Dict[str, Any]]] = Field(
        default=None, description="Epidemiological signal data and analysis"
    )
    risk_assessment: Optional[Dict[str, Any]] = Field(
        default=None, description="Overall risk assessment based on data analysis"
    )
    recommendations: Optional[List[str]] = Field(
        default=None, description="Actionable recommendations based on analysis"
    )
    trends: Optional[List[TrendResponse]] = Field(
        default=None, description="Raw epidemiological data from successful fetch_epi_signal calls"
    )
...
class PolicyDraftRequest(BaseModel):
    ...
    strategy: StrategyCreate = Field(description="The selected strategy to implement as a policy")
    alert: AlertCreate = Field(description="The original alert that triggered the strategy")
    author: Optional[str] = Field(default="AI Policy Generator", description="Author name for the policy draft")

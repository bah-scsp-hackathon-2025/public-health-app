...
class PolicyDraftGenerationAgent:
    ...
    async def generate_policy_draft(
        self, strategy: StrategyCreate, alert: AlertCreate, author: str = "AI Policy Generator"
    ) -> PolicyCreate:
        """
    ...
    def _build_policy_generation_prompt(
        self, strategy: StrategyCreate, alert: AlertCreate, policy_file_ids: List[str]
    ) -> str:
    ...
    async def _call_claude_for_policy(self, policy_prompt: str, policy_file_ids: List[str]) -> str:
        ...
        if policy_file_ids:
            ...
            for file_id in policy_file_ids:
                document_attachments.append({"type": "document", "source": {"type": "file", "file_id": file_id}})
            ...
            system_message = SystemMessage(
                content="""You are a Public Health Policy Draft Generator AI with access to official policy and strategy documents. Your task is to generate comprehensive policy drafts based on strategy and alert information.
...
You must respond with a comprehensive policy document in formal policy format. The content should be detailed, professional, and ready for governmental or organizational review and adoption."""
            ...
            try:
                response = await self.chat_anthropic.ainvoke([system_message] + messages, config=config)
            ...
            # Add system message
            system_message = SystemMessage(
                content="""You are a Public Health Policy Draft Generator AI. Generate comprehensive policy drafts based on strategy and alert information.
...
Respond with a comprehensive policy document in formal policy format."""
    ...
    def _parse_policy_from_response(
        self, response_text: str, strategy: StrategyCreate, alert: AlertCreate, author: str
    ) -> PolicyCreate:

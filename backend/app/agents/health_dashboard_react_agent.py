...
class PublicHealthReActAgent:
    ...
    def __init__(self):
        ...
        self.mcp_host = (
            ...
            if hasattr(settings, "mcp_server_host")
        ...
        if anthropic_key and anthropic_key.startswith("sk-ant-"):
            ...
            if ssl_verify:
                ...
                ca_bundle = (
                    settings.ssl_ca_bundle
                    if hasattr(settings, "ssl_ca_bundle") and settings.ssl_ca_bundle
                    else certifi.where()
                )
    ...
    def _setup_langfuse_tracing(self):
        ...
        try:
            langfuse_secret_key = (
                ...
                if hasattr(settings, "langfuse_secret_key")
                else os.getenv("LANGFUSE_SECRET_KEY")
            )
            langfuse_public_key = (
                settings.langfuse_public_key
                if hasattr(settings, "langfuse_public_key")
                else os.getenv("LANGFUSE_PUBLIC_KEY")
            )
            langfuse_host = (
                settings.langfuse_host
                if hasattr(settings, "langfuse_host")
                else os.getenv("LANGFUSE_HOST", "https://cloud.langfuse.com")
            )
            ...
            if (
                ...
                and (
                    ...
                    if hasattr(settings, "langfuse_tracing")
                    else os.getenv("LANGFUSE_TRACING", "true").lower() == "true"
                )
            ):
                self.langfuse_handler = CallbackHandler(
                    ...
                    metadata={
                        ...
                        "project": (
                            ...
                            if hasattr(settings, "langfuse_project")
                            else "public-health-dashboard"
                        ),
    ...
    async def _reasoning_node(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            system_msg = SystemMessage(
                content="""You are a Public Health Data Analyst AI Agent specializing in epidemiological surveillance and compiling data from external sources for dashboards.
...
2. "ANALYSIS_COMPLETE" if ready for final synthesis""".format(
        ...
        try:
            ...
            for tool_result in tool_results:
                ...
                # Process detect_rising_trend outputs - ALWAYS attempt to parse, regardless of has_data flag
                    ...
                    logger.debug(
                        f"🔍 Processing detect_rising_trend result (has_data={has_data}): {tool_content[:300]}..."
                    )
    ...
    def _parse_epi_signal_output(self, content: str, tool_args: Dict[str, Any] = None) -> Optional[EpiSignalData]:
        ...
        try:
            ...
            logger.debug(
                f"✅ Parsed epi signal: {signal_name} for {geo_areas or ['US']}, current_value={current_value}, trend={trend_direction}"
            )
    ...
    def _parse_trend_analysis_output(
        ...
        try:
                    ...
                    if json_match:
                        try:
                            ...
                            signal_name = trend_data.get(
                                "signal",
                                trend_data.get("signal_name", "unknown_signal"),
                            )
                            logger.debug(
                                f"✅ Extracted from JSON pattern: {rising_periods}/{total_periods} rising periods (raw: {raw_rising_periods})"
                            )
                            break
                        except json.JSONDecodeError:
                            continue
            ...
            logger.debug(
                f"✅ Parsed trend analysis: signal={signal_name}, rising={rising_periods}, total={total_periods}, risk={risk_level}"
            )
    ...
    async def _generate_alert_for_trend(
        self,
        trend_data: TrendAnalysisData,
        ...
        logger.debug(
            f"🔍 Trend data: rising={trend_data.rising_periods}, total={trend_data.total_periods}, risk={trend_data.risk_level}"
        )
        try:
            logger.debug(
                f"🚨 Generating alert for trend: {trend_data.signal_name} with {trend_data.rising_periods}/{trend_data.total_periods} rising periods"
            )
                ...
                if json_match:
                    try:
                        ...
                        if len(missing_fields) == 0:
                            ...
                            logger.debug(
                                f"✅ Generated alert: {alert_create.name} (risk score: {alert_create.risk_score})"
                            )
    ...
    def _build_alert_generation_prompt(
        ...
        if epi_signals:
            ...
            for signal in epi_signals[-3:]:  # Last 3 signals
                prompt_parts.append(
                    f"- {signal.get('display_name', 'Unknown')}: {signal.get('trend_direction', 'unknown')} trend"
                )
...
Use the structured data provided to create specific, evidence-based insights that are grounded in official public health policy."""
    ...
    def _build_final_analysis_prompt(self, state: Dict[str, Any], policy_file_ids: Optional[List[str]] = None) -> str:
        ...
        if policy_file_ids:
            prompt_parts.append(
                f"POLICY DOCUMENTS AVAILABLE: {len(policy_file_ids)} COVID-19 policy briefs are attached to this message."
            )
            prompt_parts.append(
                "Please reference these policy documents to ensure your recommendations align with official public health guidelines."
            )
            prompt_parts.append("")
...
"""
    ...
    def _format_trends_from_fetch_data(self, state: Dict) -> List[Dict]:
            ...
            try:
                ...
                if isinstance(result_str, str):
                    try:
                        ...
                        logger.debug(
                            f"Parsed JSON result with {len(parsed_result) if isinstance(parsed_result, list) else 1} data points"
                        )
        ...
        logger.debug(
            f"✅ Formatted {len(trends)} TrendResponse objects from {len(fetch_data_list)} fetch_epi_signal results"
        )
        return trends
